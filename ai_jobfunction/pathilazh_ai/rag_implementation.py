...
class PathilazhImplementation():
    ...
    '''
    def get_request_status(self, job_name):
        function_name = inspect.currentframe().f_code.co_name

        # Execute the ZCQL query and fetch the current request details
        # API codebase 
        query = f"SELECT * FROM Pathilazh WHERE Pathilazh.REQUEST_STATUS = 'Pending' AND Pathilazh.ROWID = '{job_name}';"
        ...
        if(zcql_response_code == 200):
            ...
            logging.warning({"Service":"Catalyst","Org id":org_id,"Project":project_name,"Workitem Name":workitem_name,"Path":f"{self.file_name}.{self.class_name}.{function_name}","Execution id":job_name,"Request Status":"","Log Type":"SEVERE","Log Data":f"Exception occured while fetching pending records from table.","Response":str(zcql_response.json()),"Reference":"While fetching table rows.","Reference ID":self.reference_id,"Added Time":format_time})
    ...
    '''
    def pathilazh_request_processing(self, job_name):
        function_name = inspect.currentframe().f_code.co_name
        ...
        pending_requests = self.get_request_status(job_name)
            ...
            for question_id in question_ids:
                ...
                if(question_status == "VALID"):
                    ...
                    if(rag_response_statuscode == 200):
                        ...
                        starting_string = response_message[:50]
